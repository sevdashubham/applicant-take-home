...
import { clearGiftCard, selectedGC, selectGiftCard } from '../../../slices/checkout-slice';
...
const CheckoutPanelView: React.FC = (): React.ReactElement => {
    ...
    const giftCardClickHandler = useCallback(
    ...
    const checkoutListFactory = () => {
        return giftCards.giftcard_list.map((giftCard: PrizeoutOfferValueOptions) => (
            <CheckoutItems
                ...
                giftCard={giftCard}
                isSelected={selectedGiftCard.checkout_value_id === giftCard.checkout_value_id}
                onClickHandler={giftCardClickHandler}
            />
        ));
    };
    ...
    const renderGiftCard = () => {
        ...
        return <OfferGiftCard offer={giftCards} isSelected={false} />;
    };
        ...
        <section className="checkout">
            <div className="grid grid--top-bottom grid--stretch-top">
                <div className="grid__item no-scrollbars">
                    <section className="checkout__brand">{renderGiftCard()}</section>
                </div>
                <div className="grid__item checkout-item-list">{checkoutListFactory()}</div>
