...
const DEFAULT_SELECTED_INDEX = {
    childIndex: -1,
    parentIndex: -1,
};
const emptyGiftCard = [] as PrizeoutOfferValueOptions[];
const DEFAULT_SELECTED_GC = {
    giftcard_list: emptyGiftCard,
    image_url: '',
    name: '',
};
...
export const offersSlice = createSlice({
    ...
    reducers: {
        ...
        updateSelectedOfferPosition(state, action: PayloadAction<offerPositions>) {
            ...
            if (
                parentIndex === state.selectedPosition.parentIndex &&
                childIndex === state.selectedPosition.childIndex
            ) {
...
export const selectedOfferGiftCards = ({ offers: { selectedGiftCards } }: RootState): PrizeoutOfferSelectedGC =>
    selectedGiftCards;
